...
const INICIAR_COMPONENTES = {
    ...
    async llenarTabla() {
        ...
        for (let objeto of data) {
            const { fila, categoria, temas, enfoque, objetivo, socio_negocio, tienda, sugerir_modalidad } = objeto;
            ...
            html += `<tr>
                        ...
                        <td>
                            <button type="button" data-fila="${fila}" data-bs-toggle="tooltip" data-bs-title="Eliminar" class="btn btn-link text-danger btn_eliminar_registro">
...
const EVENTOS = {
    ...
    eliminarRegistro() {
        $("#tb_registro").on("click", ".btn_eliminar_registro", async (evento) => {
            const fila = Number($(evento.currentTarget).data("fila"));

            if (!fila) {
                Swal.fire({
                    icon: "warning",
                    title: "Campo obligatorio",
                    text: "No hay un número de fila para identificar!"
                });
            ...
            Swal.fire({
                title: 'Eliminación',
                text: "¿Está seguro(a) de eliminar la fila?",
                icon: 'question',
                showCancelButton: true,
                showLoaderOnConfirm: true,
                confirmButtonColor: '#3085d6',
